/**
 * 2026-08-30 scope / slate recovery v1.
 *
 ...
 *
 *   npx tsx --env-file=.env.local scripts/audit-2026-08-30-scope-slate-recovery-v1.ts
 */
...
import {
  ...
  captureFootballFixtures20260830,
  rebuildFootballScheduleFromCapture,
} from "./capture-2026-08-30-football-fixtures-v1";

export const SLATE_RECOVERY_REL =
  "data/audits/2026-08-30-scope-slate-recovery-v1.json";
...
export const FROZEN_FORMAL_OBSERVED_AT = "2026-08-29T21:19:31.776+09:00";
...
export const OPERATOR_REVIEW_ITEMS = [
  {
    ...
    reviewNote:
      "Screenshots 1-3 display leftover 08.29(토) football matchups. Preserved in operator observations with scopeMembership=EXCLUDED_NON_TARGET_DATE. Excluded from the 2026-08-30 official Daily Scope denominator. Not used to reopen sealed 2026-08-29 artifacts.",
  },
  {
    reviewState: "OPERATOR_REVIEW_REQUIRED" as const,
    section: "MARKET_ID_GAPS_NOT_INVENTED",
    marketIdRange: [7538, 7539, 7540, 7541, 7686, 7687, 7688, 7689],
    reviewNote:
      "IDs 7538-7541 and 7686-7689 were not on any screenshot and were not invented.",
  },
  {
    reviewState: "OPERATOR_REVIEW_REQUIRED" as const,
    section: "FOOTBALL_RAW_GLYPH_에스피뇰",
    reviewNote:
      "라리가 away label stored as 에스피뇰 exactly as displayed. Not silently unified to registered alias 에스파뇰.",
  },
];

function excludedCrossDate(row: ObservedRow) {
  return {
    ...row,
    identityStatus: "EXCLUDED_NON_TARGET_DATE",
    mappingStatus: "NO_SCHEDULE_JOIN_CROSS_DATE",
    scheduleJoinStatus: "EXCLUDED_NON_TARGET_DATE",
    competitionRegistryJoin: "NOT_ATTEMPTED",
    matchId: null,
    ...
    scopeAccountingState: "EXCLUDED_NON_TARGET_DATE",
    scopeMembership: "EXCLUDED_NON_TARGET_DATE",
  };
}

function joinMlb(row: ObservedRow, schedule: MlbScheduleArtifactGame[]) {
  if (row.displayedDateKst !== DATE_KST) return excludedCrossDate(row);
...
export async function runSlateRecovery(cwd = process.cwd()) {
  ...
  const footballCaptureResult =
    await captureFootballFixtures20260830(cwd);
  ...
  const footballOfficialJoined = footballJoined.filter(
    (r) => r.displayedDateKst === DATE_KST,
  );
  const footballMatched = footballOfficialJoined.filter(
    (r) => r.identityStatus === "MATCHED",
  );
  const footballUnresolved = footballOfficialJoined.filter(
    (r) => r.identityStatus !== "MATCHED",
  );
  const footballIdentityReview = footballOfficialJoined.filter(
    (r) => r.scopeAccountingState === "IDENTITY_REVIEW_REQUIRED",
  );
  const footballCompetitionReview = footballOfficialJoined.filter(
    (r) => r.scopeAccountingState === "COMPETITION_REVIEW_REQUIRED",
  );
  ...
  const recoveryReason = scopeLockReady
    ? `Official 2026-08-30 target-date denominator is ${officialScopeTotal} of ${observedBatchCount} observed batch matchups. Cross-date 08.29 leftover football is preserved in operator observations with scopeMembership=EXCLUDED_NON_TARGET_DATE and is excluded from the official denominator. MLB exact-identity joins applied where alias+time matched. Sealed football competition aliases resolve truncated operator labels exactly. Remaining football gaps are IDENTITY_REVIEW_REQUIRED for unregistered team aliases. No basketball/volleyball in this batch. Accounting covers officialScopeTotal without guessing.`
  ...
  const nextCurrentState = {
    ...
    providerCallCount: recordedProviderCallCount,
    providerEndpoints: [
      "/api/v1/schedule?sportId=1&startDate=2026-08-29&endDate=2026-08-30&hydrate=probablePitcher",
      "/fixtures?date=2026-08-30&timezone=Asia/Seoul",
    ],
    predictionCalls: 0,
    resultCalls: 0,
    engineCalls: 0,
    ...
    nextRecommendedStep: scopeLockReady
      ? "Write 2026-08-30 Daily Scope Lock CANDIDATE, then OWNER review. Do not start B1."
  ...
  const slate = {
    ...
    providerCalls: [
      ...
      },
      {
        sport: "FOOTBALL",
        provider: "api-football",
        endpoint: "/fixtures?date=2026-08-30&timezone=Asia/Seoul",
    ...
    bySport: {
      ...
      FOOTBALL: {
        ...
        officialTargetDateScope: footballOfficialJoined.length,
