/**
 * 2026-08-30 Daily Scope Lock — FINAL SEAL after OWNER approval.
 *
 *   npx tsx --env-file=.env.local scripts/lock-2026-08-30-daily-scope-v1.ts
 *
...
export const LOCK_REL = "data/audits/2026-08-30-daily-scope-lock-v1.json";
...
export async function lockDailyScopeCandidate(cwd = process.cwd()) {
  ...
  let scopeLockedAt = sealedNow;
  let ownerSealedAt: string | null = sealedNow;
  ...
  const nextCurrent = {
    ...
    dailyScopeLockStatus: SCOPE_LOCK_STATUS,
    status: "SCOPE_LOCK_COMPLETE",
    nextRecommendedStep: "B1 schedule/identity on sealed 2026-08-30 Scope.",
  };
  await writeFile(
    recoveryAbs,
  ...
  const lock = {
    ...
    bySportReconciliation: {
      FOOTBALL: {
        ...
        scheduleMatched: footballOfficial.filter(
          (r) => r.scopeAccountingState === "SCHEDULE_MATCHED",
        ).length,
        identityReviewRequired: footballOfficial.filter(
          (r) => r.scopeAccountingState === "IDENTITY_REVIEW_REQUIRED",
        ).length,
        competitionReviewRequired: footballOfficial.filter(
          (r) => r.scopeAccountingState === "COMPETITION_REVIEW_REQUIRED",
        ).length,
        pregameEligible: footballOfficial.filter(
          (r) =>
      ...
      BASKETBALL: {
        ...
        excludedCrossDateCount: 0,
        offDateDisplayedMatchups: 0,
      },
    },
    ...
    note: "FINAL SEAL. Official 2026-08-30 Daily Scope denominator is target-date rows only. Twenty 08.29 leftover football matchups remain in structured operator observations with scopeMembership=EXCLUDED_NON_TARGET_DATE and are excluded from this denominator. Sealed 2026-08-29 artifacts were not reopened. Football competition aliases resolved 29/29. Twenty-seven official football rows remain IDENTITY_REVIEW_REQUIRED and stay in the denominator. Not a Prediction input.",
    nextRecommendedStep: "B1 schedule/identity on sealed 2026-08-30 Scope.",
  };
