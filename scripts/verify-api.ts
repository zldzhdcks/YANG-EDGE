 ...
 */
import { fetchTodayGames } from "../src/lib/api/games";
...
async function main() {
  ...
  const { fetchTodayGames: fetchGamesAgain } = await import(
    "../src/lib/api/games"
  );
  // Note: getInternalAppBaseUrl reads env at call time, so override works for new calls
  // But modules already loaded - getInternalAppBaseUrl reads process.env each call, OK
