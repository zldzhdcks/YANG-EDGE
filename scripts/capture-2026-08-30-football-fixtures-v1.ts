/**
 * Persist the 2026-08-30 API-Football date fixture dump for slate recovery.
 *
 * Uses an existing captured file if present.
 * Makes one getFixtures call only when the dump is missing.
 *
 *   npx tsx --env-file=.env.local scripts/capture-2026-08-30-football-fixtures-v1.ts
 */
...
export const FIXTURES_CAPTURE_REL =
  "data/research/football/2026-08-30-fixtures-captured-v1.json";
...
export async function captureFootballFixtures20260830(cwd = process.cwd()) {
...
async function main() {
  const result = await captureFootballFixtures20260830();
  console.log(
    JSON.stringify(
      {
