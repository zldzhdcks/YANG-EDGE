...
export function validateTarget(t:Target){exactKeys(t,targetKeys);requireRule([39,140,135,78].includes(t.leagueId),'LEAGUE_IDENTITY');requireRule([2023,2024,2025,2026].includes(t.season),'SEASON_FIREWALL');for(const id of [t.fixtureId,t.homeTeamId,t.awayTeamId])requireRule(Number.isSafeInteger(id)&&id>0,'INVALID_IDENTITY');requireRule(t.homeTeamId!==t.awayTeamId,'SAME_TEAM');requireRule(Number.isFinite(Date.parse(t.kickoffUtc))&&new Date(t.kickoffUtc).toISOString()===t.kickoffUtc,'INVALID_KICKOFF');}
...
export function eligible(rows:Match[],t:Target){validateTarget(t);const cutoff=observedCutoff(t);const selected=order(rows.filter(r=>r.leagueId===t.leagueId&&r.season<=t.season&&r.fixtureId!==t.fixtureId&&Date.parse(r.kickoffUtc)<cutoff&&Date.parse(r.kickoffUtc)>=cutoff-365*DAY));validatePool(selected);return selected;}
export function validateHistory(t:Target,rows:Match[]){validateTarget(t);validatePool(rows);const cutoff=observedCutoff(t);for(const r of rows)requireRule(r.leagueId===t.leagueId&&r.season<=t.season&&r.fixtureId!==t.fixtureId&&Date.parse(r.kickoffUtc)<cutoff&&Date.parse(r.kickoffUtc)>=cutoff-365*DAY,'TEMPORAL_OR_LEAGUE_LEAKAGE');}
