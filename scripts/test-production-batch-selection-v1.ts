...
test('bound input rejects wrong batch, changed hash and unsafe chronology',async t=>{const a=await setup(t),s=selectProductionBatch(a.root,a.date,'one');const b={batchId:'one',scopeId:s.binding!.scopeId,scopeSha256:s.binding!.scopeSha256};const observed=new Date(Date.now()-10000).toISOString();const f=envelope({observedAt:observed,collectedAt:observed}),h=envelope({sealedAt:observed});write(join(s.root,'fixture.json'),f);write(join(s.root,'history.json'),h);const inputs={fixtureEvidencePath:'fixture.json',fixtureEvidenceHash:f.sha256,historyPath:'history.json',historyHash:h.sha256};const p={...b,targetId:'one',inputSnapshotId:`${b.scopeSha256}:one`,inputSha256:sha(canonical({fixtureHash:f.sha256,historyHash:h.sha256,targetId:'one',...b})),observedAt:observed,collectedAt:observed,asOf:observed,predictionCutoff:new Date(Date.parse(a.start)-60000).toISOString(),scheduledStart:a.start};validateBatchInputProof(s.root,b,'one',a.start,p,inputs);validateBatchInputProof(s.root,s.binding!,'one',a.start,p,inputs);assert.throws(()=>validateBatchInputProof(s.root,b,'one',a.start,{...p,batchId:'two'},inputs));assert.throws(()=>validateBatchInputProof(s.root,b,'one',a.start,{...p,inputSha256:'0'.repeat(64)},inputs));assert.throws(()=>validateBatchInputProof(s.root,b,'one',a.start,{...p,collectedAt:new Date(Date.parse(a.start)+1).toISOString()},inputs));assert.throws(()=>validateBatchInputProof(s.root,b,'one',a.start,undefined,inputs));});
