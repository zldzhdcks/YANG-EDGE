...
import {validateTarget,validateHistory,order,DAY,requireRule,type Target,type Match} from '../football-poisson-v2-draw-research/evaluation-v1/contracts-evaluation-v1';
...
export function selectTargets(target:Target,rows:Target[]){
  ...
  return order(rows.filter(r=>r.leagueId===target.leagueId&&r.season<=target.season&&r.fixtureId!==target.fixtureId&&Date.parse(r.kickoffUtc)>=cutoff-365*DAY&&Date.parse(r.kickoffUtc)+2*DAY<cutoff));
}
