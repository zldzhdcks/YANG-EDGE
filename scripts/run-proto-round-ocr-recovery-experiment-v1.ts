/**
 * Isolated OCR recovery experiment v1 runner.
 * Pilot10 Discovery evidence only. Does not replace production OCR.
 * Does not open Fresh Validation pixels or Holdout.
 *
 *   npm run run:proto-round-ocr-recovery-experiment-v1 -- --year 2026 --round 105 --json
 */
...
import {
  ...
  canonicalJson,
  createWindowsCropOcrProvider,
  cropOcrProviderVersion,
  FRESH_VALIDATION_SEAL_FILE_NAME,
  FROZEN_FIRST_SHOT_EXTRACTION_SHA256,
  identifyFreshUnseenImages,
  OCR_RECOVERY_EXPERIMENT_RESULT_FILE_NAME,
  OCR_RECOVERY_LOCAL_DIR_NAME,
...
function selectionPilotKeys(raw: unknown): FrozenIdentityV1[] {
  const doc = raw as { discoveryRowKeys?: FrozenIdentityV1[] };
...
function asPilotTruthRecords(raw: unknown): PilotTruthV1[] {
  const doc = raw as { records?: PilotTruthV1[] };
...
function visualGeometryFromDocument(raw: unknown): VisualRowGeometryV1[] {
  const doc = raw as {
    images?: Array<{
...
function semanticGeometryFromDocument(raw: unknown): SemanticRegionGeometryV1[] {
  const doc = raw as {
    rows?: Array<{
      sourceImageSha256: string;
      visualRowIndex: number;
...
async function main() {
  ...
  const firstShotSourceImages = (rawOcr.images ?? []).map((img) => ({
    sourceImageSha256: img.sourceImageSha256,
    sourceFileName: img.sourceFileName,
  }));
  const canonicalImages = manifest.files
  ...
  const seal = buildFreshValidationSeal({
    ...
    createdFromIntakeManifest: INTAKE_MANIFEST_FILE_NAME,
    images: fresh.images,
  });
  const localDir = path.join(yangAbs, OCR_RECOVERY_LOCAL_DIR_NAME);
  const sealAbs = path.join(localDir, FRESH_VALIDATION_SEAL_FILE_NAME);
  ...
  const result = await runOcrRecoveryExperimentV1({
    protoRoundKey: manifest.meta.protoRoundKey,
    ...
    cropOcr: createWindowsCropOcrProvider(),
  });

  const resultAbs = path.join(localDir, OCR_RECOVERY_EXPERIMENT_RESULT_FILE_NAME);
  await writeJsonAtomic(resultAbs, {
    ...result,
    ocrProviderVersion: cropOcrProviderVersion(),
  ...
  const payload = {
    action: "ocr-recovery-experiment-v1",
    protoRoundKey: manifest.meta.protoRoundKey,
    FIRST_SHOT_EXTRACTION_SHA256: extractionSha,
    intake: {
      physicalFileCount: manifest.summary.physicalFileCount,
      canonicalImageCount: manifest.summary.canonicalImageCount,
      duplicateExactCount: manifest.summary.duplicateExactCount,
      unsupportedFileCount: manifest.summary.unsupportedFileCount,
      extractionEligibleCount: manifest.summary.extractionEligibleCount,
    },
    freshUnseenImageCount: fresh.freshUnseenImageCount,
    freshUnseenImageSha256: fresh.freshUnseenImageSha256,
    freshUnseenFileNames: fresh.freshUnseenFileNames,
    FRESH_VALIDATION_SEAL_SHA256: sealSha,
    ...
    BEST_OCR_RECOVERY_CANDIDATE: result.bestCandidate,
    deltaVsBaseline: {
      participantLeftExactEvidencePresent:
        best.participantLeftExactEvidencePresent - baseline.participantLeftExactEvidencePresent,
      participantRightExactEvidencePresent:
        best.participantRightExactEvidencePresent - baseline.participantRightExactEvidencePresent,
      numericCellsAllExactEvidencePresent:
        best.numericCellsAllExactEvidencePresent - baseline.numericCellsAllExactEvidencePresent,
      allPrimaryEvidencePresent:
        best.allPrimaryEvidencePresent - baseline.allPrimaryEvidencePresent,
    },
    taxonomyBaseline: result.taxonomyByCandidate.BASELINE_WHOLE_SCREEN,
    taxonomyBest: result.taxonomyByCandidate[result.bestCandidate],
    PARSER_CHANGED: false,
    PRODUCTION_RAW_OCR_CHANGED: false,
    GROUND_TRUTH_USED: "PILOT10_DISCOVERY_ONLY",
    DISCOVERY_11_TO_30_ANNOTATED: false,
    HOLDOUT_READ: false,
    FRESH_VALIDATION_READ_FOR_RULE_DESIGN: false,
  ...
  console.log(`BEST_OCR_RECOVERY_CANDIDATE=${result.bestCandidate}`);
  console.log(`FRESH_VALIDATION_SEAL_SHA256=${sealSha}`);
  console.log(`freshUnseenImageCount=${fresh.freshUnseenImageCount}`);
}
