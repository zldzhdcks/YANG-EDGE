/**
 * Persist the 2026-08-29 API-Football date fixture dump for slate recovery.
 *
 * Uses an existing captured file if present.
 * Makes one getFixtures call only when the dump is missing.
 *
 *   npx tsx --env-file=.env.local scripts/capture-2026-08-29-football-fixtures-v1.ts
 */
...
export const FIXTURES_CAPTURE_REL =
  "data/research/football/2026-08-29-fixtures-captured-v1.json";
...
export async function captureFootballFixtures20260829(cwd = process.cwd()) {
...
async function main() {
  const result = await captureFootballFixtures20260829();
  console.log(
    JSON.stringify(
      {
