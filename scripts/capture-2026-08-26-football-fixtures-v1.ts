/**
 * Persist the 2026-08-26 API-Football date fixture dump for B1.1 recovery.
 *
 * Uses an existing captured file if present.
 * Makes one getFixtures call only when the dump is missing.
 *
 *   npx tsx --env-file=.env.local scripts/capture-2026-08-26-football-fixtures-v1.ts
 */
...
export const DATE_KST = "2026-08-26";
...
export async function captureFootballFixtures20260826(cwd = process.cwd()) {
  const existing = loadFootballFixtureCapture(cwd);
  if (existing && existing.fixtureCount > 0 && Array.isArray(existing.fixtures)) {
...
async function main() {
  const result = await captureFootballFixtures20260826();
  console.log(
    JSON.stringify(
      {
