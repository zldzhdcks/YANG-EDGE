...
export function validateTarget(t:Target){exactKeys(t,targetKeys);requireRule([39,140,135,78].includes(t.leagueId),'LEAGUE_IDENTITY');requireRule(t.season===2023,'EVALUATION_FIREWALL');for(const id of [t.fixtureId,t.homeTeamId,t.awayTeamId])requireRule(Number.isSafeInteger(id)&&id>0,'INVALID_IDENTITY');requireRule(t.homeTeamId!==t.awayTeamId,'SAME_TEAM');requireRule(Number.isFinite(Date.parse(t.kickoffUtc))&&new Date(t.kickoffUtc).toISOString()===t.kickoffUtc,'INVALID_KICKOFF');}
...
export function eligible(rows:Match[],t:Target){validateTarget(t);const cutoff=Date.parse(t.kickoffUtc)-1;const selected=order(rows.filter(r=>r.leagueId===t.leagueId&&r.fixtureId!==t.fixtureId&&Date.parse(r.kickoffUtc)+2*DAY<cutoff&&Date.parse(r.kickoffUtc)>=cutoff-365*DAY));validatePool(selected);return selected;}
export function validateHistory(t:Target,rows:Match[]){validateTarget(t);validatePool(rows);const cutoff=Date.parse(t.kickoffUtc)-1;for(const r of rows)requireRule(r.leagueId===t.leagueId&&r.fixtureId!==t.fixtureId&&Date.parse(r.kickoffUtc)+2*DAY<cutoff&&Date.parse(r.kickoffUtc)>=cutoff-365*DAY,'TEMPORAL_OR_LEAGUE_LEAKAGE');}
