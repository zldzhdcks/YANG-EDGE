...
export function validateTarget(t:Target){exactKeys(t,targetKeys);requireRule([39,140,135,78].includes(t.leagueId),'LEAGUE_IDENTITY');requireRule([2023,2024,2025].includes(t.season),'SEASON_FIREWALL');for(const id of [t.fixtureId,t.homeTeamId,t.awayTeamId])requireRule(Number.isSafeInteger(id)&&id>0,'INVALID_IDENTITY');requireRule(t.homeTeamId!==t.awayTeamId,'SAME_TEAM');requireRule(Number.isFinite(Date.parse(t.kickoffUtc))&&new Date(t.kickoffUtc).toISOString()===t.kickoffUtc,'INVALID_KICKOFF');}
