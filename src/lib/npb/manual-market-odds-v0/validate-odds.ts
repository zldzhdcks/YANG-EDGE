/**
 * Decimal moneyline odds validation — presentation helpers only.
 * Implied probability is NOT model / prediction probability.
 */
...
/** Raw implied probability from decimal odds (includes overround). Not model p. */
...
export function normalizeImpliedPair(
  ...
  if (!(sum > 0)) {
    return { away: awayRaw, home: homeRaw };
  }
