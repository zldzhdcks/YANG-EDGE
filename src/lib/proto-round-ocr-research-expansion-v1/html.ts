...
export function renderDiscovery2AnnotationHtml(input: {
  protoRoundKey: string;
  records: Discovery2AnnotationRecordV1[];
}): string {
  const payload = {
...
<head>
  ...
  <title>Discovery-2 Human Annotation v1</title>
  <style>
...
<body>
  <header>
    <div>
      <h1>Discovery-2 사람 전사</h1>
      <div class="meta">원본 스크린샷 픽셀만 봅니다. 기계 인식 결과와 후보는 표시하지 않습니다. 시장 표시가 화면에 명확히 보이지 않으면 비워 두세요.</div>
    </div>
    <div class="progress">
  ...
  <main>
    ...
    <section>
      <form id="truthForm" autocomplete="off">
        ...
        <p class="note">Export JSON 파일명은 discovery2-annotation-v1.json 입니다. 평가는 아직 하지 않습니다.</p>
      </form>
    </section>
  </main>
  <script>
    const PACK = ${escapeHtmlScriptJson(payload)};
    const STORAGE_KEY = "proto-round-ocr-research-expansion-discovery2-v1:" + PACK.protoRoundKey;
    ...
    document.getElementById("exportJson").onclick = () => {
      ...
      a.download = ${JSON.stringify(DISCOVERY2_ANNOTATION_FILE_NAME)};
      a.click();
