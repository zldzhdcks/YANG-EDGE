/**
 * v2-B TRAIN-only median imputation + TRAIN-only standardization.
 * 23 remaining base features. 22 missing indicators stay 0/1 and are not scaled.
 * restDays.missing is still produced from the sealed Feature row even though
 * restDaysBefore is not in X.
 */
...
export type LogisticTrainRowV2b = {
  gamePk: number;
  officialDate: string;
...
export type LogisticPreprocessorV2b = {
  fitPartition: "TRAIN";
...
export function medianOfV2b(values: number[]): number {
...
export function extractRawBaseAndMissingV2b(
  feature: MlbIndependentFeatureRowV1,
): { base: Array<number | null>; missing: number[] } {
  const names = orderedLogisticBaseFeatureNamesV2b();
  const missingNames = orderedLogisticMissingIndicatorNamesV2b();
  ...
  if (
    base.length !== MLB_INDEPENDENT_LOGISTIC_BASE_DIM_V2B ||
    missing.length !== MLB_INDEPENDENT_LOGISTIC_MISSING_DIM_V2B
  ) {
    throw new IndependentLogisticError(
...
export function fitTrainPreprocessorV2b(
  trainRows: LogisticTrainRowV2b[],
): LogisticPreprocessorV2b {
  if (trainRows.length === 0) {
    throw new IndependentLogisticError("EMPTY_TRAIN", "TRAIN rows empty");
  }
  const orderedBaseFeatureNames = orderedLogisticBaseFeatureNamesV2b();
  const orderedMissingIndicatorNames = orderedLogisticMissingIndicatorNamesV2b();
  const orderedModelFeatureNames = orderedLogisticModelFeatureNamesV2b();
  const columns: Array<Array<number | null>> = orderedBaseFeatureNames.map(
    () => [],
  );
  for (const row of trainRows) {
    const { base } = extractRawBaseAndMissingV2b(row.feature);
  ...
  for (let j = 0; j < orderedBaseFeatureNames.length; j += 1) {
    ...
    medianByFeature[orderedBaseFeatureNames[j]!] = medianOfV2b(observed);
  }
...
export function transformRowV2b(
  feature: MlbIndependentFeatureRowV1,
  prep: LogisticPreprocessorV2b,
): number[] {
  const { base, missing } = extractRawBaseAndMissingV2b(feature);
  const out = new Array<number>(MLB_INDEPENDENT_LOGISTIC_MODEL_DIM_V2B);
  for (let j = 0; j < MLB_INDEPENDENT_LOGISTIC_BASE_DIM_V2B; j += 1) {
  ...
  for (let j = 0; j < MLB_INDEPENDENT_LOGISTIC_MISSING_DIM_V2B; j += 1) {
    out[MLB_INDEPENDENT_LOGISTIC_BASE_DIM_V2B + j] = missing[j]!;
  }
  return out;
}

export function transformMatrixV2b(
  ...
  for (let i = 0; i < n; i += 1) {
    const vec = transformRowV2b(rows[i]!.feature, prep);
    X.set(vec, i * dim);
    y[i] = rows[i]!.target;
  }
  return { X, y };
