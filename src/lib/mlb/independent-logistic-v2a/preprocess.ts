/**
 * v2-A TRAIN-only median imputation + TRAIN-only standardization.
 * 29 remaining base features. 22 missing indicators stay 0/1 and are not scaled.
 */
...
export type LogisticTrainRowV2a = {
  gamePk: number;
  officialDate: string;
...
export type LogisticPreprocessorV2a = {
  fitPartition: "TRAIN";
...
export function medianOfV2a(values: number[]): number {
...
export function extractRawBaseAndMissingV2a(
  feature: MlbIndependentFeatureRowV1,
): { base: Array<number | null>; missing: number[] } {
  const names = orderedLogisticBaseFeatureNamesV2a();
...
export function fitTrainPreprocessorV2a(
  trainRows: LogisticTrainRowV2a[],
): LogisticPreprocessorV2a {
  if (trainRows.length === 0) {
    throw new IndependentLogisticError("EMPTY_TRAIN", "TRAIN rows empty");
  }
  const orderedBaseFeatureNames = orderedLogisticBaseFeatureNamesV2a();
  const orderedMissingIndicatorNames = orderedLogisticMissingIndicatorNamesV2a();
  const orderedModelFeatureNames = orderedLogisticModelFeatureNamesV2a();
  const columns: Array<Array<number | null>> = orderedBaseFeatureNames.map(
    () => [],
  );
  for (const row of trainRows) {
    const { base } = extractRawBaseAndMissingV2a(row.feature);
  ...
  for (let j = 0; j < orderedBaseFeatureNames.length; j += 1) {
    ...
    medianByFeature[orderedBaseFeatureNames[j]!] = medianOfV2a(observed);
  }
...
export function transformRowV2a(
  feature: MlbIndependentFeatureRowV1,
  prep: LogisticPreprocessorV2a,
): number[] {
  const { base, missing } = extractRawBaseAndMissingV2a(feature);
  const out = new Array<number>(MLB_INDEPENDENT_LOGISTIC_MODEL_DIM_V2A);
  for (let j = 0; j < MLB_INDEPENDENT_LOGISTIC_BASE_DIM_V2A; j += 1) {
  ...
  for (let j = 0; j < MLB_INDEPENDENT_LOGISTIC_MISSING_DIM_V2A; j += 1) {
    out[MLB_INDEPENDENT_LOGISTIC_BASE_DIM_V2A + j] = missing[j]!;
  }
  return out;
}

export function transformMatrixV2a(
  ...
  for (let i = 0; i < n; i += 1) {
    const vec = transformRowV2a(rows[i]!.feature, prep);
    X.set(vec, i * dim);
    y[i] = rows[i]!.target;
  }
  return { X, y };
