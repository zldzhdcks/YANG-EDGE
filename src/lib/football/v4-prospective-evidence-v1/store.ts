...
import {PREGAME_SCHEMA,readPregame} from './projection';
...
export async function exactEvidence(root:string,evidenceId:string,expectedSha:string){const e=JSON.parse((await get(root,evidenceId,expectedSha)).toString()) as Envelope;requireProof((e.schemaVersion==='football-v4-prospective-evidence-v1'||e.schemaVersion===PREGAME_SCHEMA)&&e.evidenceId===evidenceId,'ENVELOPE_IDENTITY');if(e.schemaVersion===PREGAME_SCHEMA)readPregame(e,{providerFixtureId:e.providerFixtureId,homeTeamId:''});else requireProof(sha(stable((e.payload as {raw:unknown}).raw))===e.sourceArtifactSha256,'SOURCE_HASH_MISMATCH');return e;}
