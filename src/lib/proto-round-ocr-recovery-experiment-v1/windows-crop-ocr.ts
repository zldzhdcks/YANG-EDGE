...
import type { CropOcrExtractResultV1, CropOcrProviderV1, CropOcrRequestV1 } from "./types";
...
function assertSafeCropRequest(request: CropOcrRequestV1): CropOcrRequestV1 {
  const { crop, scale, grayscale } = request;
  if (
    ...
    throw new OcrRecoveryExperimentError("INVALID_CROP_BOX");
  }
  if (scale !== 1 && scale !== 2 && scale !== 3) {
    throw new OcrRecoveryExperimentError("INVALID_CROP_SCALE");
  }
  if (typeof grayscale !== "boolean") {
    throw new OcrRecoveryExperimentError("INVALID_GRAYSCALE_FLAG");
  }
  if (typeof request.imagePath !== "string" || request.imagePath.trim() === "") {
    throw new OcrRecoveryExperimentError("INVALID_IMAGE_PATH");
  }
  return request;
}

function buildRecognizeCropScript(request: CropOcrRequestV1): string {
  const spec = assertSafeCropRequest(request);
  const gray = spec.grayscale ? "$true" : "$false";
  return `
...
try {
  ...
  if (${gray}) {
    $grayBitmap = [Windows.Graphics.Imaging.SoftwareBitmap]::Convert(
      $bitmap,
  ...
  $payload = @{
    ok = $true
    providerVersion = ('Windows.Media.Ocr crop-v1 ' + $engine.RecognizerLanguage.LanguageTag + '; OS ' + [System.Environment]::OSVersion.Version.ToString())
...
function parseExtractJson(stdout: string): CropOcrExtractResultV1 {
  const parsed = JSON.parse(stdout) as CropOcrJson;
  if (parsed.ok !== true) {
    throw new OcrRecoveryExperimentError(parsed.error || "WINDOWS_MEDIA_CROP_OCR_FAILED");
  }
  return {
...
export function createWindowsCropOcrProvider(): CropOcrProviderV1 {
  return {
    async extractCrop(request) {
      ...
      const stdout = await new Promise<string>((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
          child.kill();
          reject(new OcrRecoveryExperimentError("WINDOWS_MEDIA_CROP_OCR_TIMEOUT"));
        }, 60_000);
        ...
        child.on("close", (code) => {
          ...
          if (!out.trim()) {
            reject(
              new OcrRecoveryExperimentError(
...
export function cropOcrProviderVersion(): string {
  return `Windows.Media.Ocr crop-v1; OS ${os.release()}`;
}
