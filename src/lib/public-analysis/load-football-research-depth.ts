 ...
 for(const d of details){
  ...
  try{if(source){
   ...
   for(const r of ctx.receipts){
    ...
    assert(p.teams.some(t=>t.id===Number(r.params.team)),'QUERY_TEAM');
    ...
    for(const row of raw.response){assert(row.fixture.id!==p.fixtureId,'TARGET_EXCLUDED');assert(row.fixture.status.short==='FT','COMPLETED_ONLY');assert(row.league.id===p.leagueId&&row.league.season===p.season,'ROW_SCOPE');assert(row.teams.home.id===Number(r.params.team)||row.teams.away.id===Number(r.params.team),'ROW_TEAM');assert(Date.parse(row.fixture.date)<Date.parse(r.providerFetchedAt),'FUTURE_ROW');}
    for(const row of raw.response)history.push({providerFixtureId:row.fixture.id,leagueId:row.league.id,season:row.league.season,kickoffUtc:row.fixture.date,homeTeamId:row.teams.home.id,awayTeamId:row.teams.away.id,fixtureStatus:'FT',fullTimeHomeGoals:row.score.fulltime.home,fullTimeAwayGoals:row.score.fulltime.away,providerFetchedAt:r.providerFetchedAt,sourceHash:r.sha256});
   }
   assert(p.teams.every(t=>ctx.receipts.some((r:any)=>Number(r.params.team)===t.id)),'BOTH_TEAMS_REQUIRED');
  ...
  const gate=depthGate(research?.teams??null,!!research);
